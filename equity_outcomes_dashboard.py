...
class EquityOutcomesAnalysisDashboardDashboard:
    ...
    def connect_database(self) -> bool:
        try:
            ...
            result = self.conn.execute(
                "SELECT COUNT(*) as count FROM main_main_analytics.v_equity_outcomes_by_demographics"
            ).fetchall()
    ...
    def load_data(self) -> dict:
        try:
            data = {}
            query = "SELECT * FROM main_main_analytics.v_equity_outcomes_by_demographics LIMIT 100"
    ...
    def build_layout(self, data: dict):
        self.app.layout = html.Div([
            html.Div([
                html.H1("Equity Outcomes Analysis Dashboard", style={"color": "#fff", "textAlign": "center", "paddingTop": "20px"}),
...
if __name__ == "__main__":
    dashboard = EquityOutcomesAnalysisDashboardDashboard()
    dashboard.run(port=8052, debug=True)
